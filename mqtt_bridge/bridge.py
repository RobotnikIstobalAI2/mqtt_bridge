...
def create_bridge(
    ...
    **kwargs
    ...
    # replace this with ROS2 once a solution for this esists
    ...
    return factory(
        ...
        topic_to=topic_to, msg_type=msg_type,
        frequency=frequency,
        **kwargs
    )
...
class RosToMqttBridge(Bridge):
    ...
    bridge ROS messages on `topic_from` to MQTT topic `topic_to`.
    expect `msg_type` ROS message type.
    """

    def __init__(
        self, topic_from: str,
        topic_to: str,
        msg_type, frequency: Optional[float] = None,
        **kwargs
    ):
...
class MqttToRosBridge(Bridge):
    ...
    bridge MQTT messages on `topic_from` to ROS topic `topic_to`.
    MQTT messages will be converted to `msg_type`.
    """
